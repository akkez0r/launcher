...
public class PanelCrashReport extends Panel {
	public PanelCrashReport(CrashReport var1) {
		...
		if(var3.contains("Pixel format not accelerated")) {
			...
			var6 = var6 + "      -----------------------      \n";
			var6 = var6 + "\n";
			var6 = var6 + "This build was unable to start because it failed to find an accelerated OpenGL mode.\n";
			...
			var6 = var6 + "      CMC client has crashed!      \n";
			var6 = var6 + "      -----------------------      \n";
			var6 = var6 + "\n";
			var6 = var6 + "The client stopped because it encountered a problem; " + var1.getDescription() + "\n\n";
			File var7 = var1.getFile();
			if(var7 == null) {
				var1.saveToFile(new File(new File(Minecraft.getMinecraftDir(), "crash-reports"), "crash-" + (new SimpleDateFormat("yyyy-MM-dd_HH.mm.ss")).format(new Date()) + "-client.txt"), Minecraft.getMinecraft().getLogAgent());
				var7 = var1.getFile();
			}
			...
			if(var7 != null) {
				...
				var5 = "Full report at:\n" + var8 + "\nShare that full file when reporting this crash — not only this dialog.\n\n" + var5;
			} else {
