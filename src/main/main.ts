...
async function requestAuthApi<T>(
  ...
  try {
    response = await fetch(`${CMC_API_BASE_URL}${endpoint}`, {
      method,
      headers,
    ...
    throw new Error(
      `Unable to reach CMC auth server at ${CMC_API_BASE_URL}. Start cmc-auth or set CMC_API_BASE_URL to your backend URL.`
    );
  }
...
function resolveCmcApiBaseUrl(): string {
  const rawValue =
    process.env.CMC_API_BASE_URL?.trim() || "https://auth.craviorsmp.com";
  let parsed: URL;
  try {
...
/**
 * LaunchWrapper may remap legacy launcher username skin lookups. CMC skins use HTTPS UUID URLs on the auth API.
...
function injectCmcIdentityArgs(currentArgs: string[], user: AuthUser, accessToken: string): string[] {
  ...
  const withUuid = upsertLaunchArg(withUsername, "--uuid", user.cmcUuid);
  return upsertLaunchArg(withUuid, "--accessToken", accessToken);
}
...
async function startHostServer(): Promise<{ ok: boolean; message: string }> {
  ...
  const serverProps = ensureHostServerProperties(serverRoot);
  if (!serverProps.ok) {
    return { ok: false, message: serverProps.message };
  }
...
function escapeRegExpForServerPropsKey(rawKey: string): string {
  return rawKey.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
}

function mergeServerProperty(content: string, key: string, value: string): string {
  const re = new RegExp(`^\\s*${escapeRegExpForServerPropsKey(key)}\\s*=.*$`, "gm");
  let found = false;
  const merged = content.replace(re, () => {
    found = true;
    return `${key}=${value}`;
  });
  if (found) {
...
function getCmcApiBaseForServerProps(): string {
  const explicit = process.env.CMC_API_BASE_URL?.trim();
  if (!explicit) {
    return "https://auth.craviorsmp.com";
  }
  try {
    return resolveCmcApiBaseUrl();
  } catch {
    return "https://auth.craviorsmp.com";
  }
}

function ensureHostServerProperties(serverRoot: string): { ok: boolean; message: string } {
  const gameDir = path.join(serverRoot, "game");
  const propsPath = path.join(gameDir, "server.properties");
  ...
  try {
    ...
    if (existsSync(propsPath)) {
      ...
      content =
        "# Generated by Akkez Launcher host mode\n" +
        "# CMC: online-mode=false — external Minecraft-account verification is not used on this build.\n" +
        "server-port=25565\n" +
        "max-players=20\n" +
        "motd=Akkez CMC Server\n";
    }

    content = content.replace(/^\s*online-mode\s*=.*$/gm, "online-mode=false");
    if (!/^\s*online-mode\s*=/m.test(content)) {
      content = `${content.trimEnd()}\nonline-mode=false\n`;
    }
    if (!content.includes("CMC: online-mode=false")) {
      content =
        "# CMC: online-mode=false — external Minecraft-account verification is not used on this build.\n" +
        content.trimStart();
    }

    content = mergeServerProperty(content, "cmc-api-base-url", getCmcApiBaseForServerProps());
    content = mergeServerProperty(content, "cmc-require-account", "true");

    writeFileSync(propsPath, content, "utf8");
    return { ok: true, message: "Host server properties prepared." };
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unknown server.properties error";
    return { ok: false, message: `Failed to prepare server.properties: ${message}` };
  }
}
